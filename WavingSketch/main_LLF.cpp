...
vector<Data> all_to;
ofstream fout("LLF_res.txt", ios::app);
HashMap mp;
StreamMap sp;
...
void Test_Hitter(string PATH)
{
    ...
    for (int i = 2; i <= 20; i += 2)
    {
        ...
        sketch[0] = new WavingSketch_LLF<16, 8>(memory, HIT, BF_memory);
...
void Test_Speed(string PATH)
{
    ...
    for (int i = 15; i <= 20; ++i)
    {
        ...
        {
            WavingSketch_LLF<16, 8> sketch(memory, HIT, BF_memory);
            int num = 0;
            cout << "Num: " << num << endl;
            TP start = now();
            num = all_from.size();
