...
#define sffr_sf 200
#define FR_IBLT (sffr_iblt + sffr_sf)  //raw flowradar memory

#define thres_insert 18    //sf insert_thres
#define thres_evict 2   //sf evict
...
// load data
int load_data() {
    const char *filename="../datasets/webdocs_form01.dat";
...
void demo_flowradar(int packet_num /*, ofstream &oFile*/)
{
    ...
    printf("\nFlowRadar with SC: %d KB FR + %d KB SC", sffr_iblt, sffr_sf);
    ...
    auto * sf_fr_a = new SF_InsertableIBLT<sffr_iblt*1024, sffr_sf*1024, 16,       32, 32,         thres_insert, thres_evict>();
    auto * sf_fr_b = new SF_InsertableIBLT<sffr_iblt*1024, sffr_sf*1024, 16,       32, 32,         thres_insert, thres_evict>();
    clock_gettime(CLOCK_MONOTONIC, &dtime1);
    ...
    printf("\tFlowRadar with LadderFilter: %d KB FR + %d KB LadderFilter", sffr_iblt, sffr_sf);
    ...
    printf("\tFlowRadar with Log: %d KB FR + %d KB Log", sffr_iblt, sffr_sf);
